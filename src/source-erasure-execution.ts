import {sourceErasureBatches,sourceErasureInput,sourceQuoteProblems,applySourceQuoteRepairs,decodeSourceErasureResponse,decodeSourceErasure,SOURCE_ERASURE_PROMPT,SOURCE_QUOTE_REPAIR_PROMPT,type sourceErasureWork} from './source-erasure.js';
...
export async function executeSourceErasure(work:Work,workers:number,signal:AbortSignal,call:Call):Promise<SourceErasurePlan>{
 ...
 const offsets:number[]=[];let count=0;for(const defects of problems){offsets.push(count);count+=defects.length;}
 if(count>8)throw new ServiceError('EVIDENCE_VALIDATION','Too many source quote defects for one global repair');
 if(count){
  const PROBLEMS=problems.flatMap((defects,batch)=>defects.map((p,local)=>({index:offsets[batch]!+local,...p})));let raw:unknown;
  try{raw=await call(SOURCE_QUOTE_REPAIR_PROMPT,JSON.stringify({PROBLEMS}),shared,'source_erasure_repair');}
  ...
  const byIndex=new Map<number,any>();for(const r of patch.repairs){
   if(!r||typeof r!=='object'||Object.keys(r).some(k=>!['index','status','quote'].includes(k))||!Number.isInteger(r.index)||r.index<0||r.index>=count||byIndex.has(r.index)||r.status!=='resolved'||typeof r.quote!=='string'||!r.quote)throw new ServiceError('EVIDENCE_VALIDATION','Invalid global source quote repair');byIndex.set(r.index,r);
  }
  for(const [index,batch] of batches.entries())if(problems[index]!.length)raws[index]=applySourceQuoteRepairs(raws[index],batch,{repairs:problems[index]!.map((_,local)=>({...byIndex.get(offsets[index]!+local),index:local}))});
 }
