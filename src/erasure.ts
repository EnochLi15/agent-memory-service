...
import {canonical,sameSlot,sameScope,scopeKey,ServiceError,type AddRequest,type Fact,type Operation,type ErasureBoundary,type ErasurePlan} from './types.js';
...
export function erasureWork(req:AddRequest,prior:Fact[],incoming:Fact[],operations:Operation[],existing:ErasureBoundary[]){
 ...
 const candidates:{fact_id:string;key:string;fact:ReturnType<typeof meaning>;boundary:ErasureBoundary;scope_matches:boolean;authorization:unknown}[]=[];
