...
/** First vertical slice: explicit rejection of an unrecorded assistant assertion
 * in this add. Historical and fact-linked targets stay on the ordinary pipeline.
 * The complete chunk (including later echoes) is reviewed, never a keyword subset. */
export function sourceOperationWork(req:AddRequest,facts:Fact[]){
 ...
 const targets=req.messages.flatMap((m,index)=>m.role==='assistant'&&!speakerPrefix(m.content.trim())?[{slot:index,message:index,text:m.content}]:[]);
 const enabled=instructions.length>0&&targets.some(t=>instructions.some(i=>t.message<i.message));
 return {fingerprint:digest({protocol:'source-operation-v1',req,facts}),enabled,instructions,targets,request:req,facts};
}
export function sourceOperationInput(work:ReturnType<typeof sourceOperationWork>){
 const input={INSTRUCTIONS:work.instructions,TARGETS:work.targets,NEW_MESSAGES:work.request.messages,EXISTING_FACTS:work.facts.map(f=>({id:f.id,content:f.content,subject:f.subject,predicate:f.predicate,scope:f.scope,state:f.state}))};
...
export function decodeSourceOperations(raw:unknown,work:ReturnType<typeof sourceOperationWork>):SourceOperationPlan{
 ...
 for(const d of ds){
  ...
  for(const slot of d.target_slots){const t=work.targets.find(t=>t.slot===slot);if(!t||t.message>=i.message||!d.cuts.some(c=>c.message===t.message))throw new ServiceError('EVIDENCE_VALIDATION','Source target must precede its instruction and be cut');}
  ...
  for(const c of d.cuts){const m=work.request.messages[c.message],start=m?.content.indexOf(c.quote)??-1;
...
export function validateSourceOperations(plan:SourceOperationPlan|undefined,req:AddRequest,oldFacts:Fact[],newFacts:Fact[],messages:StoredMessage[]){
 const work=sourceOperationWork(req,oldFacts);
 ...
 if(sourceRejectionFindings(plan,req,newFacts).length)throw new ServiceError('EVIDENCE_VALIDATION','Proposed facts reintroduce rejected source-only details');
 for(const d of checked.decisions.filter(d=>d.action==='reject_source'))for(const c of d.cuts){
  const m=messages[c.message];if(!m||m.content!==req.messages[c.message]?.content||m.id!==createHash('sha256').update(`${req.user_id}\0${req.request_id}\0${c.message}`).digest('hex'))throw new ServiceError('EVIDENCE_VALIDATION','Source operation message identity changed');
  ...
  for(const f of [...oldFacts,...newFacts].filter(f=>f.state!=='erased')){
   if(f.source_spans?.some(s=>s.source_id===m.id&&s.start<range.end&&s.end>range.start)||f.source_ids.includes(m.id)&&!f.source_spans?.length)throw new ServiceError('EVIDENCE_VALIDATION','Source-only rejection overlaps a fact witness; ordinary erasure review required');
  }
  cuts.set(m.id,[...(cuts.get(m.id)??[]),range]);
 }
 return {work,plan:checked,cuts};
}
export function resolvedSourceInstructions(plan:SourceOperationPlan|undefined,req:AddRequest,facts:Fact[]){
 const work=sourceOperationWork(req,facts);
...
export function sourceRejectionFindings(plan:SourceOperationPlan|undefined,req:AddRequest,facts:Pick<Fact,'content'|'value'|'subject'|'predicate'|'scope'|'time_text'>[]):string[]{
 const rejected=plan?.decisions.filter(d=>d.action==='reject_source')??[];if(!rejected.length)return [];
 const cuts=rejected.flatMap(d=>d.cuts),removed=new Set(tokens(cuts.filter(c=>req.messages[c.message]?.role==='assistant').map(c=>c.quote).join(' ')));
 const human=new Set(tokens(req.messages.flatMap((m,index)=>{
