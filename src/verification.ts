import {canonical,sameScope,ServiceError,type AddRequest,type Extraction,type Fact} from './types.js';
...
export function forgetScopeContext(proposal:Extraction,facts:Fact[]){
 ...
 })).map(f=>({id:f.id,subject:f.subject,predicate:f.predicate,scope:f.scope,content:f.content,value:f.value,state:f.state,modality:f.modality,source_quotes:f.source_quotes,depends_on:f.depends_on,supersedes:f.supersedes,forget_operation_indices:proposal.operations.flatMap((op,index)=>op.type==='forget'&&op.target_ids.includes(f.id)?[index]:[])}))}));
}
export function verificationInput(req:AddRequest,proposal:Extraction,facts:Fact[],unrepresented:number[],scope?:VerificationScope){
 ...
 return {...(forgetContext.length?{FORGET_SCOPE_CONTEXT:forgetContext}:{}),NEW_MESSAGES:req.messages.map((m,index)=>({index,...m})),PARTICIPANT_INDEX:participantIndices(req),MESSAGE_SOURCE_LINKS:messageSourceLinks(req,proposal),CHECK_SCOPE:required,REPLACEMENT_TARGETS:required.replacements,PROPOSAL:indexedProposal(proposal,req),TARGET_FACTS:facts.filter(f=>targetIds.has(f.id)).map(f=>({id:f.id,subject:f.subject,predicate:f.predicate,scope:f.scope,...(f.scopeHash?{scopeHash:f.scopeHash,matching_scope_operation_indices:proposal.operations.flatMap((o,index)=>sameScope(o,f)?[index]:[])}:{}),content:f.content,value:f.value,state:f.state,modality:f.modality,depends_on:f.depends_on,supersedes:f.supersedes})),OMISSION_HINTS:unrepresented};
}
