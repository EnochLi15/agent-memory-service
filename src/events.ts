...
export function projectEvents(events:MemoryEvent[],facts:Fact[],visible:Set<string>):Fact[]{
 ...
 return events.map(e=>{
  ...
  const action=forgotten?`${e.category} entry was explicitly removed from memory (forgotten)`:`${e.type} ${e.category}`;
  const actor=e.actor==='user'?'The user':e.actor==='participant'?'The source participant':null;
  const authorization=actor&&forgotten?`${actor} explicitly asked the memory service to forget the ${e.category}. `:actor&&e.type==='restore'?`${actor} explicitly authorized remembering the new ${e.category} again. `:'';
