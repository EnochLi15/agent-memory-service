...
export function modelFailure(error:unknown,signal:AbortSignal,streamStarted:boolean,finishReason:string|null,refusalDetected=false){
 ...
 const categories=error instanceof ServiceError&&error.code==='MODEL_OUTPUT'?(finish==='content_filter'||refusalDetected?'provider_filtered':finish==='length'||error.message==='Model output too large'?'output_limit':'incomplete_output'):
  error instanceof ServiceError?'service_validation':error instanceof SyntaxError?'invalid_json':
  signal.aborted?(signal.reason?.name==='TimeoutError'?'deadline':'cancelled'):
 ...
 return {error_type:errorType,error_category:categories,http_status:http,stream_started:streamStarted,finish_reason:finish,refusal_detected:refusalDetected,transport_code:code??null};
}
