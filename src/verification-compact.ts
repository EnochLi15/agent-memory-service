 ...
 for(const name of arrayNames){
  ...
  for(const [position,row] of values.entries()){
   ...
   if(name==='fact_checks'){
    ...
    if(!fact||!scope.fact_indices.includes(index)||![4,5].includes(row.length)||typeof row[1]!=='boolean'||typeof row[2]!=='boolean'||(row.length===5&&row[4]!==null&&typeof row[4]!=='string')){invalid('Invalid compact fact tuple');continue;}
    ...
    if(!op||!scope.operation_indices.includes(index)||![3,4].includes(row.length)||typeof row[1]!=='boolean'||typeof row[2]!=='boolean'||(row.length===4&&row[3]!==null&&typeof row[3]!=='string')){invalid('Invalid compact operation tuple');continue;}
    ...
    if(!replacement||![2,3].includes(row.length)||typeof row[1]!=='boolean'||(row.length===3&&row[2]!==null&&typeof row[2]!=='string')){invalid('Invalid compact replacement tuple');continue;}
