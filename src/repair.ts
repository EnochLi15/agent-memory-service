...
import {missingForgetObligations,forgetObligations,authorizesForget} from './operation-intent.js';
...
export const SOURCE_AUTHORIZATION_REPAIR_PROMPT=`\nSOURCE_AUTHORIZATION_CANDIDATES lists literal direct retirement spans from each rejected operation's original message. These are candidate citations, not authorization for that target or missing-coverage obligations. Use a candidate only when the full original context authorizes deletion of that exact target, property and scope; otherwise remove only the unsupported operation. A candidate may already support a valid sibling concerning a different property, but that never authorizes reuse for the rejected target. Do not expand targets, borrow another message or modify valid siblings. Every source edit still requires target binding, chronology and independent semantic verification within the unchanged REPAIR_SCOPE.`;
...
/** Candidate witnesses only; never assign a source or infer its target scope.
 * Omit an oversized hint entirely instead of truncating an exact source quote. */
export function sourceAuthorizationCandidates(req:AddRequest,proposal:Extraction,scope:RepairScope){
 const candidates:{operation_index:number;source:{index:number;quote:string}}[]=[],obligations=forgetObligations(req);
 for(const operation_index of new Set(scope.operation_indices)){
  const op=proposal.operations[operation_index];
  if(!op||op.type!=='forget'||authorizesForget(op,req)||!scope.source_indices.includes(op.source.index))continue;
  for(const {index,span} of obligations){
   if(index!==op.source.index)continue;
   candidates.push({operation_index,source:{index,quote:span.quote}});
   if(candidates.length>8||JSON.stringify(candidates).length>4096)return [];
  }
 }
 return candidates;
}
