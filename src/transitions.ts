...
Return {decisions:[{index:number,relation:compatible|exclusive|uncertain,old_source_slot:number,new_source_slot:number,reason:string}]}. Include each candidate exactly once. Select old_source_slot and new_source_slot as zero-based indexes into the corresponding fact's source_quotes arrays. The server resolves the original quotes; do not copy or paraphrase witness text. Choose sources supporting the owner and relevant qualifier. Explain the actual semantic relationship. Never edit facts, dates or targets. Explicit operations and explicitly verified replacement edges are handled separately.`;
export const TRANSITION_RESPONSE_FORMAT={type:'json_schema' as const,json_schema:{name:'state_transition_v3',strict:true,schema:{type:'object',properties:{decisions:{type:'array',items:{type:'object',properties:{index:{type:'integer'},relation:{type:'string',enum:['compatible','exclusive','uncertain']},old_source_slot:{type:'integer'},new_source_slot:{type:'integer'},reason:{type:'string'}},required:['index','relation','old_source_slot','new_source_slot','reason'],additionalProperties:false}}},required:['decisions'],additionalProperties:false}}};
