...
Output protocol source-first-coverage-tuples-v1 extends the compact tuples only as specified here. ${SOURCE_COVERAGE_SEMANTICS}For a message containing only such incidental memorable context, use [message_index,"source_backed",[candidate_slots],"brief explanation why no additional structured item is required"]. For mixed structured and incidental content use [message_index,"represented",[fact_indices],[operation_indices],[candidate_slots]]; an optional final explanation string is allowed and cannot override the source references or completeness judgment. The supplied raw slots must cover ALL incidental content not already covered by referenced facts/operations. An empty slot list proves nothing. Never select another speaker/message, a command, or a candidate absent from the supplied catalog. Ordinary represented/not_memorable/missing rows retain their existing semantics. All necessary facts, operations and replacements still receive their normal independent checks. Missing core content remains missing even if safe raw candidates exist.`;
