...
type GenerationPurpose='extraction'|'verification'|'repair'|'rerank'|'erasure_binding';
...
export class Models {
  ...
  private stageModel(purpose:GenerationPurpose):string{return purpose==='rerank'?this.config.llmModel:this.config.llmStageModels[purpose==='erasure_binding'?'verification':purpose]??this.config.llmModel;}
  ...
  async json(system: string, user: string, signal: AbortSignal,context?:GenerationContext): Promise<unknown> {
    ...
    const model=this.stageModel(purpose),structured=['verification','erasure_binding'].includes(purpose)&&this.config.verificationResponseFormat==='json_schema';
    const formatAudit=['verification','erasure_binding'].includes(purpose)?{verification_response_format:structured?'json_schema':'json_object'}:{};let last:unknown;
    for(let attempt=0;attempt<2;attempt++){
      ...
      try{
        const stream=await this.client.chat.completions.create({
          ...
          response_format:structured?(purpose==='erasure_binding'?ERASURE_RESPONSE_FORMAT:COMPACT_VERIFICATION_RESPONSE_FORMAT):{type:'json_object'},max_completion_tokens:10000,stream:true,stream_options:{include_usage:true},
        },{signal});
