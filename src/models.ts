...
export class Models {
  ...
  async verify(proposal:Extraction,req:AddRequest,facts:Fact[],omitted:number[],signal:AbortSignal,session=new VerificationSession(),resolvedSourceActions:unknown[]=[]):Promise<string[]>{
    ...
    for(let attempt=0;attempt<2;attempt++){
      let raw:unknown;
      try{raw=await this.json(prompt,input+repair,signal,{purpose:'verification',verification_format:compact?COMPACT_VERIFICATION_PROTOCOL:'verbose',verification_scope:counts,trace:{user_id:req.user_id,request_id:req.request_id}});}
