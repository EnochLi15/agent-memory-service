...
export interface Config {
  ...
  embeddingBase: string; embeddingModel: string; embeddingDimensions: number; embeddingSpace: string;
...
export function configFromEnv(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  return {
    ...
    embeddingBase: (env.MEMORY_EMBEDDING_BASE_URL ?? 'http://127.0.0.1:11434').replace(/\/$/, ''), embeddingModel: model,
