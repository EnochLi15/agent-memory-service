...
test('v9 validates routing at atomic commit and refuses reuse of v8 storage',async()=>{
 ...
 try{const x=new Extractor(config,models as any),before=store.snapshot('s'),p=await x.prepare(request,before,AbortSignal.timeout(3000));assert.equal(p.sourceFormat,'dual-source-v9-s1');const corrupt=structuredClone(p);delete corrupt.sourceOperationPlan!.route_review;assert.throws(()=>store.commit(request,hash(JSON.stringify(request)),corrupt,0),/Missing source routing/);assert.deepEqual(store.snapshot('s'),before);assert.throws(()=>store.commit(request,hash(JSON.stringify(request)),p,0,'indexes'));assert.deepEqual(store.snapshot('s'),before);store.commit(request,hash(JSON.stringify(request)),p,0);assert.equal(store.meta('source_format'),'dual-source-v9-s1');assert.doesNotMatch(JSON.stringify(store.snapshot('s').erasureSources),/Peak Gym/);
...
test('atomic v9 commit rejects an all-ordinary proof with omitted operations before any mutation',()=>{
 ...
 const prepared={sourceFormat:'dual-source-v9-s1',sourceOperationPlan:ordinarySourceRoute(w,review),facts:[],operations:[],messages:[],anchor:null,degraded:[],embeddingSpace:'test'};
