...
async function fixture(fn:any){
 ...
 const prepare=async(r:any,proposal:any,decision?:any)=>new Extractor(config,{verify:async()=>[],embedBatch:async(xs:string[])=>xs.map(()=>[1,0]),json:async(_s:string,input:string,signal:AbortSignal,context:any)=>{
  if(context?.purpose!=='erasure_binding')return structuredClone(proposal);calls++;const d=JSON.parse(input);d.CANDIDATES=d.CANDIDATES.map((c:any)=>({...d.FACTS[c.fact_slot],...d.BOUNDARIES[c.boundary_slot],...c}));return decision?decision(d):{decisions:d.CANDIDATES.map((c:any,index:number)=>({index,effect:c.fact.predicate==='coworker_name'||c.fact.predicate==='project_code'?'retain':'erase',quote:c.fact.source_quotes[0],reason:c.fact.predicate==='project_code'?'A separate explicitly named project code.':c.fact.predicate==='coworker_name'?'A different real person in work context.':'The same backup name restated under another label.'}))};}
