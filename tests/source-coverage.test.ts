...
test('v10 real preparation flow requires independent raw coverage and atomic indexed witnesses',async()=>{
 ...
 try{const before=store.snapshot('s'),p=await new Extractor(config,models).prepare(req,before,AbortSignal.timeout(3000));assert.ok(sawPrompt);assert.equal(p.sourceFormat,'dual-source-v10');assert.equal(p.facts.length,0);assert.ok(p.sourceCoveragePlan);
 ...
 assert.throws(()=>store.commit(req,hash(JSON.stringify(req)),p,0,'indexes'));assert.deepEqual(store.snapshot('s'),before);store.commit(req,hash(JSON.stringify(req)),p,0);assert.equal(store.meta('source_format'),'dual-source-v10');assert.ok(store.lexicalPassages('relieved',5).length);assert.equal(store.revision(),1);
