...
test('fabricated source is replaced only by grounded recovery; unknown operation ID is rejected',()=>fixture(async({s,c}:any)=>{
 ...
 const bad=new Extractor({...c,mode:'enhanced'},{verify:async()=>[],json:async(system:string)=>system.includes('PATCH_SCHEMA')?{}:({facts:[],operations:[{type:'forget',target_ids:['other-tenant-id'],subject:'user',predicate:'hobby',source:{index:0,quote:'I like hiking.'}}]})} as any);await assert.rejects(()=>bad.prepare(req,s.snapshot('s'),AbortSignal.timeout(1000)),/Unknown memory operation target/);assert.equal(s.revision(),0);
}));
...
test('partially recovered raw evidence becomes invisible when a linked fact is superseded or erased',()=>fixture(async({s,c,add,find}:any)=>{
 ...
 const x=new Extractor({...c,mode:'enhanced'},{verify:async()=>[],json:async(system:string)=>system.includes('PATCH_SCHEMA')?{}:({facts:[{content:'The user lives in Seattle.',subject:'user',predicate:'current_city',value:'Seattle',cardinality:'single',sources:[{index:0,quote:'My current city is Seattle.'}]},{content:'I enjoy kayaking.',subject:'user',predicate:'hobby',value:'kayaking',sources:[{index:0,quote:'I enjoy kayaking.'}]}],operations:[]}),embedBatch:async()=>{throw Error('offline')}} as any);
