...
每个用户在 `MEMORY_DATA_DIR/sha256(user_id)/memory.sqlite` 保存数据，WAL与FULL同步；增强发布来源格式为 `dual-source-v10`，离线为 `dual-source-v2`。不同来源格式、未知格式或无版本非空数据在打开租户库时拒绝。更换来源格式或向量空间需使用兼容快照，或新目录重新灌入。不要静默迁移旧库。
