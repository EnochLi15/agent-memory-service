...
Source candidates include whole messages, including unlinked assistant replies, and related fact content. Exact value matches and hashed non-stopword anchors nominate candidates; the verification model then distinguishes the authorized erased information from independent people, contexts and neighboring clauses. Source work is bounded to 256 candidates / 256000 total transmitted JSON input characters across all batches (including tables repeated across batches); each model batch is bounded to 64 candidates / 64000 JSON input characters. A single oversized candidate rejects the request. Batches use local indexes mapped back into one complete commit plan and share the same original deadline. Failure or uncertainty in any batch rejects the entire write. The source_erasure_v4 input stores exact source payloads and boundary/authorization payloads once in SOURCES and BOUNDARIES; every candidate retains its own index, references and matching words. No semantic fields are dropped, and batch sizing uses the actual packed JSON input. The source_erasure_v4 model response uses whole-source erase/retain decisions or unique nonoverlapping erase_quotes for a mixed source. Uniform decisions do not copy the text; mixed decisions explicitly certify every remaining gap as independent and retained. The model reports a fixed reason category consistent with the effect instead of a repeated free-text explanation. The server reconstructs the entire original message as exact erase/retain substrings. Unknown, ambiguous, overlapping or Unicode-splitting quotes reject; mixed fact records still require review. Missing coverage, changed text, split Unicode characters, uncertainty and stale plans reject the write. Mixed fact claims require review rather than automatic content rewriting.

The source phase shares the existing total model deadline. It is logged as `source_erasure`; strict mode uses its own JSON schema. Each batch accepts up to 64 candidates / 64,000 packed JSON input characters, within the complete-work limits above; exceeding capacity fails explicitly. There is no semantic resampling or offline bypass.

`MEMORY_SOURCE_ERASURE_WORKERS` selects 1 (default), 2 or 3 classification workers. More than one requires enhanced source erasure. All batches retain full source/boundary payloads and must resolve semantic decisions before a single global quote-repair call, capped at eight defects across the entire source phase. Defects in different batches share that one call; an uncertain semantic verdict in any batch prevents repair. Local candidate and repair indices map deterministically into the complete global plan. Any worker failure cancels and drains its siblings; child signals remain bound to the original request deadline. Final partition and erased-witness consistency checks still gate the atomic commit. Worker count is part of ingestion identity, requiring fresh comparison ingestion.
...
Optional `MEMORY_EXTRACTION_WORKERS=2|3` enables bounded participant-group extraction for enhanced `message_groups` or `source_refs` requests with at least four participant messages. Default is 1. Source character balancing assigns each participant exactly once; every shard still receives the complete original messages, prior context and existing memory candidates. All groups merge chronologically before global target binding, independent verification, bounded repair and one atomic commit. Cross-shard operations use selectors; invented foreign new-fact handles reject. Any shard failure cancels and drains its siblings and rejects preparation, without publishing partial groups or entering offline recovery for that failure. Semantic repairs remain global. Full context repeats across calls, so token cost can increase. Storage format follows the representation options; the worker count is part of ingestion identity and requires fresh ingestion for comparison. The shared request deadline is unchanged.

### Joint source erasure (experimental)

`MEMORY_SOURCE_ERASURE_GROUPED=true` requires enhanced source erasure and defaults to false. It groups all nominated boundaries for the same exact source, kind and context into one indivisible review. Fixed linked erased facts are mandatory obligations; a local consistency check rejects an untouched witness before returning a plan. The joint partition expands to every original candidate for the existing atomic transaction. Original workload limits, full source context, one global quote repair, cancellation and commit guards remain. This mode changes ingestion identity and requires fresh ingestion for comparisons; it does not change the v10 storage layout or prove benchmark quality.

Joint source wire protocol v2 losslessly shares complete boundary/authorization objects within each batch; each source keeps all boundary references and its own matching words. References are local to a batch, with complete tables repeated where necessary. No source, linked-fact obligation, quote or authorization is truncated. Batch capacity counts the actual packed wire size. Changing this protocol still requires a new ingestion comparison.

Extraction shards validate complete participant ownership and cross-shard handles before merging. A missing `facts` or `operations` array remains absent in the merged proposal and is rejected by the strict grouped decoder, which invokes the existing bounded global format repair. No empty array is inferred; no worker is rerun independently. Non-array fields, omitted/foreign/duplicate groups and foreign handles still reject. Successful repair must pass full decoding, binding, semantic verification and atomic commit under the original deadline and repair limit.

Deterministic raw-span fallback is applied only to facts newly erased in the current transaction. Historical erased IDs still seed dependency invalidation, but their original broad spans must not be reapplied over already committed partial source cuts. This preserves independent mixed-source neighbors on subsequent ordinary writes and assistant echoes; fresh nonlexical erasures still receive fallback cleanup.

Verification and valid flat-patch repair inputs label each fact with `fact_index` and each operation with `operation_index`, derived from the full current arrays even for sparse rechecks. These server-generated labels exist only on model input; response schemas, patch permissions, stored facts and source indexes are unchanged. Malformed grouped proposals stay unmodified for full-schema repair. Labels reduce counting ambiguity but cannot prove semantic index/reason consistency; use fresh ingestion and real model controls to assess effectiveness.

Parallel extraction preserves typed provider connection/timeouts and HTTP 408/409/429/5xx as `EXTRACTION_UNAVAILABLE` (503) after cancelling and draining sibling shards. It neither publishes partial results nor falls back to offline success. Clients may apply their existing bounded whole-request retry policy; successful receipts remain idempotent. Schema, semantic, unknown and permanent provider errors are not made retryable by matching their message text.

Model transport retries wait 500ms before the second attempt and 1000ms before an optional third, unless the provider supplies a valid short `retry-after-ms` or `Retry-After` hint. Waits are cancellable by the original request signal; hints longer than 5 seconds stop the internal retry instead of retrying earlier than requested. Permanent HTTP errors, malformed JSON, completed semantic decisions and local validation failures do not enter this retry path. Inputs, stage model and response format remain identical; no SDK retries run outside this policy. Retry waits have separate bounded audit records and are not counted as provider generations.

`MEMORY_MODEL_TRANSPORT_ATTEMPTS=1|2|3` defaults to 2. The 3-attempt option is an explicit availability experiment under the same overall request deadline and semantic repair limit; record its additional provider attempts and cost, and use fresh ingestion/configuration identity. It does not add another semantic verification vote or enable cross-request checkpoint/resume.
