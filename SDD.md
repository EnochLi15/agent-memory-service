...
![系统架构：写入、检索、按用户隔离的 SQLite 与可选模型服务](docs/images/system-architecture.png)

图 1　系统总体架构。虚线表示可选模型调用；默认 offline 不调用模型。各用户使用独立 SQLite，事实与向量同库存储。
...
默认最多返回 100 条，同时不超过请求 top_k，并受服务配置的 6000 估算 token 预算限制。100 条对应赛题的返回上限；6000 是本方案可调的默认预算，未经参数对照评测证明为最优。`options` 中的字符串仅辅助已有证据匹配。返回字段为 `id`、`content`、`score`、`created_at`，最终答案由评测平台生成。
...
![跨会话更新与定向遗忘：保留合法历史及无关记忆](docs/images/memory-lifecycle.png)

图 2　默认规则配置下的状态变化示例。更新保留合法历史，定向遗忘作用于目标信息及其依赖。
...
![从问题到证据包：召回、排序、去重与来源覆盖](docs/images/evidence-retrieval.png)

图 3　按问题组织记忆证据。可选向量召回由运行配置控制，返回内容包含来源、状态和时间信息。

服务默认最多返回 100 条证据，并遵守请求 top_k。token 预算独立配置，当前默认 6000 估算 token，实际返回条数可能因此少于 100。条数和长度限制用于控制下游记忆上下文；具体预算应结合下游作答模型的可用上下文容量及对照评测确定。
...
- 原文留存有助于保真，也会增加噪声；返回条数上限和 token 预算可能裁掉长尾证据。
