...
全文索引采用 `porter unicode61`。启用 Embedding 时，向量与模型标识、维度和权重摘要对应；不兼容的来源格式或向量空间需要独立数据目录。
...
| 资源 | 配置模型 | 实际用途与调用条件 |
| --- | --- | --- |
| 提取模型 | gpt-5.5 | enhanced Add：提取事实与操作提案 |
| 核验模型 | gpt-5.5 | enhanced Add：来源、操作授权、擦除范围及状态转移核验 |
| 修复模型 | gpt-5.5 | enhanced Add：对未通过项生成有限范围的修复补丁 |
| 通用模型 | gpt-5.4-mini | 阶段模型未指定时的默认值；也用于可选重排和查询焦点处理，两种随包配置均关闭这两项 |
...
默认配置使用 `dual-source-v2-s1`，模型配置使用 `dual-source-v5-s1`，两者使用不同的数据卷。来源格式不匹配时拒绝访问对应租户数据库；恢复需匹配代码、配置和数据。

Docker 多阶段构建安装并编译依赖，运行阶段使用非 root 用户及持久卷。Health 检查引擎与 SQLite/FTS 就绪。普通 HTTP 日志记录请求关联、阶段、耗时和错误码，正文与凭据不进入这些日志。
...
模型适配、核验协议和检索评分各自独立，可替换模型或增加候选通道。当前采用单进程、单 SQLite Worker；进一步扩大数据规模需处理大租户扫描和工作线程竞争。开源复用及改造范围见 [UPSTREAM.md](UPSTREAM.md)。
