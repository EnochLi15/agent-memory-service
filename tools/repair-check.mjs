...
for(const run of runs){
 ...
 for(const req of reqs.filter(r=>!process.env.REPAIR_MATCH||r.request_id.includes(process.env.REPAIR_MATCH))){
  const original=new TenantStore(process.env.REPAIR_DATA_DIR??'.data/dev-v2',req.user_id);const snapshot=original.snapshot(req.session_id);const prefix='.data/repair-check-'+Date.now();const folder=join(prefix,hash(req.user_id));mkdirSync(folder,{recursive:true});await original.db.backup(join(folder,'memory.sqlite'));original.close();const store=new TenantStore(prefix,req.user_id);const start=Date.now();
